...
'use client'
import { motion } from 'framer-motion'
import { Download, Mail, ArrowDown, Github, Linkedin, Eye } from 'lucide-react'
import { useEffect, useState } from 'react'
...
const Hero = () => {
  ...
  return (
    <section className="min-h-screen flex items-center justify-center px-3 sm:px-4 lg:px-6 bg-gray-900 relative overflow-hidden">
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-gray-900 via-blue-900/20 to-gray-900"></div>
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10 pt-16">
        ...
        >
          ...
          {/* Typing animation - scaled to 0.75x */}
          <motion.div
            className="text-xl sm:text-2xl lg:text-3xl font-medium text-gray-300 mb-6 min-h-[2.5rem] flex items-center justify-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.5 }}
          >
          ...
          {/* Description - scaled to 0.75x */}
          <motion.p
            className="text-base sm:text-lg text-gray-400 mb-9 max-w-2xl mx-auto leading-relaxed px-3"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.8 }}
          >
          ...
          {/* Buttons - Fixed Icon/Text Alignment */}
          <motion.div
            ...
            transition={{ delay: 1.1 }}
          >
           {/* Updated Button with better icon */}
            <motion.a
              href="https://drive.google.com/file/d/1iD3zU7WLCQWo2ds5Go9KZTH85V87BByD/view?usp=sharing"
              target="_blank"
              rel="noopener noreferrer"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-black font-semibold px-6 py-3 rounded-full transition-all duration-300 shadow-lg hover:shadow-cyan-500/25"
            >
              <FileText size={18} />
              View Resume
            </motion.a>
            ...
            <motion.a
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="flex items-center gap-2 border-2 border-cyan-500 text-cyan-500 hover:bg-cyan-500 hover:text-black font-semibold px-6 py-3 rounded-full transition-all duration-300"
            >
              <Mail size={18} />
              Contact Me
            </motion.a>
          ...
          >
            <motion.a
              ...
              rel="noopener noreferrer"
              whileHover={{ scale: 1.2, y: -5 }}
              className="p-3 bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl text-gray-400 hover:text-cyan-400 hover:border-cyan-500/50 transition-all duration-300"
            >
              <Github size={20} />
            </motion.a>
            <motion.a
        ...
        {/* Scroll indicator - scaled to 0.75x */}
        <motion.div
