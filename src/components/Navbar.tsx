  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-3">
      ...
      >
        ...
        >
          <div className="flex justify-between items-center">
            {/* Logo - scaled to 0.75x */}
            <motion.div
              whileHover={{ scale: 1.05 }}
              className="text-lg font-bold bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 bg-clip-text text-transparent cursor-pointer"
            >
              {'<Dinesh />'}
            </motion.div>

            {/* Desktop Navigation - scaled to 0.75x */}
            <div className="hidden md:flex items-center space-x-1">
              {navItems.map((item, index) => {
                const isActive = activeSection === item.id
                return (
                  <motion.a
                    ...
                    whileTap={{ scale: 0.95 }}
                    className={`
                      relative px-3 py-2 rounded-xl font-medium cursor-pointer
                  ...
                  >
                    ...
                    {/* Hover glow effect */}
                    <motion.div
                      className="absolute inset-0 rounded-xl bg-gradient-to-r from-cyan-400/20 to-blue-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                      style={{ zIndex: -1 }}
                    />
            ...
            {/* Mobile Menu Button - scaled to 0.75x */}
            <div className="md:hidden">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="p-2 rounded-xl bg-white/10 border border-white/20 text-white hover:bg-white/15 hover:text-cyan-400 transition-all cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
              >
                <motion.div
                  animate={{ rotate: isMobileMenuOpen ? 180 : 0 }}
                  transition={{ duration: 0.3 }}
          ...
          {isMobileMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, height: 0, y: -20 }}
              transition={{ duration: 0.4, ease: "easeOut" }}
              className="md:hidden mt-3 p-3 bg-white/5 rounded-xl border border-white/10 overflow-hidden"
              style={{
                backdropFilter: 'blur(40px)',
                WebkitBackdropFilter: 'blur(40px)',
              }}
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  <motion.a
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`
                      block py-2 px-3 rounded-lg font-medium cursor-pointer text-sm
                      transition-all duration-300 group relative
                      ${isActive 
                        ? 'text-cyan-400 bg-cyan-400/10 border-l-4 border-cyan-400' 
                        : 'text-white/90 hover:text-white hover:bg-white/10'
